...
const paramQBIXArguments = require('./arguments-claiming-qbix.js');
...
async function main() {
	...
	console.log(
		...
		deployer_claiming_qbix.address
	);

	console.log("Account balance:", (await ethers.provider.getBalance(deployer_claiming_qbix.address)).toString());
    
    const claimingTokenF = await ethers.getContractFactory("ClaimingToken");
    const claimingTokenQBIX = await claimingTokenF.connect(deployer_claiming_qbix).deploy(...paramQBIXArguments);
    await claimingTokenQBIX.waitForDeployment();

    
    data_object.claimingTokenQBIX = claimingTokenQBIX.target;
	//---
    ...
    console.log("claimingTokenQBIX deployed at:", claimingTokenQBIX.target);
    ...
    if (networkName == 'hardhat') {
        ...
        await hre.run("verify:verify", {
            address: claimingTokenQBIX.target,
            constructorArguments: [...paramQBIXArguments],
        });
    }
